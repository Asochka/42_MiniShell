...
static char	*ft_dollar_question(void)
{
	...
	while (return_val[k])
		en = ft_strjoin_mod1(en, return_val[k++]);
	return (en);
}
...
static char	*ft_check_dollar(char *str, int *j, char *to_copy, t_llist *envp)
{
	...
	{
		*j = ++i;
		return (ft_strjoin_clean(str, ft_dollar_question()));
	}
	while (to_copy[i] != 34 && to_copy[i] != 39 && \
	to_copy[i] && to_copy[i] != '$')
	{
		en = ft_strjoin_mod1(en, to_copy[i]);
		i++;
	}
	val = ft_get_value_envp(&envp, en);
	if (val)
		str = ft_strjoin_clean(str, val);
	free(en);
	*j = i;
	return (str);
}
	...
	while (t)
	{
		...
		while (((char *)t->content)[i])
		{
			...
			(((char *)t->content)[i] == '$' && !((char *)t->content)[i + 1]))
				str = ft_strjoin_mod1(str, ((char *)t->content)[i++]);
		}
		free_help(&t, &str);
	}
	return (0);
